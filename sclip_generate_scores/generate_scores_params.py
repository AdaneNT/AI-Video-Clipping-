import numpy as np
from sclip_generate_scores.knapsack_implementation import knapsack

def generate_summary(all_shot_bound, all_scores, all_nframes, all_positions, d_min,d_sec, fps=30):
    # Convert minutes and seconds to total seconds
    total_duration_seconds = (d_min * 60) + d_sec
    ...
    for video_idx in range(len(all_scores)):
        ...
        # Convert the desired duration in seconds to frames
        max_frames = int(total_duration_seconds * fps)

        # Select the best shots using the knapsack implementation
        selected = knapsack(max_frames, shot_lengths, shot_imp_scores, len(shot_lengths))
