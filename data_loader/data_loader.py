...
import json
import numpy as np
from data_loader.fragments import compute_fragments
...
class CustomVideoData(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.mode == 'test':
            ...
            return frame_features
#
def get_loader(name, mode, split_index):
    vd = VideoData(name, mode, split_index)
    if mode.lower() == 'train':
        return DataLoader(vd, batch_size=1, shuffle=True)
    else:
        return vd

def get_loader_custom_video_data(mode, save_path):
    vd = CustomVideoData(mode, save_path)
    if mode.lower() == 'train':
        return DataLoader(vd, batch_size=1, shuffle=True)
    else:
        return vd
