...
class Summarizer(nn.Module):
    def __init__(self, input_size, hidden_size, num_layers=2):
        super().__init__()
        self.attn = MultiHeadSelfAttention(hidden_size)  # Updated to use MultiHeadAttention
        self.vae = VAE(input_size, hidden_size, num_layers)

    def forward(self, image_features, uniform=False):
        if not uniform:
            scores, weighted_features = self.attn(image_features)  # [seq_len, attention_size]
            # attention scores
